...
- **L16** `navigator.share` and the clipboard write must happen in the same task as the tap.
  `prepack` exists solely to satisfy this.
...
## C2. CONFIRMED — the caption is silently not copied, and the app says it was
...
**Fix:** hoist the clipboard write above the early return so it always happens on the real tap.
